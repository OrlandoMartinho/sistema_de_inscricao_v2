...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $rup = htmlspecialchars(trim($_POST['rup']));
    $curso_nome = null;
    ...
    foreach ($uploads as $key => $upload) {
        if ($upload === false || !isset($upload['conteudo']) || empty($upload['conteudo'])) {
    ...
    if ($conn->connect_error) {
        $_SESSION['error_message'] = "Erro de conexão com o banco de dados: " . $conn->connect_error;
        header("Location: inscricao.php");
        exit();
    }
    ...
    $stmt = $conn->prepare("INSERT INTO inscricoes 
        (nome_completo, email, telefone, bi_numero, curso, sexo, curso_id, foto_passe, foto_passe_type, documento_bi, documento_bi_type, comprovativo, comprovativo_type, data_de_nascimento, rup) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    // Vincular parâmetros
    $null = null;
    $foto_passe_type = $uploads['foto_passe']['tipo'];
    $documento_bi_type = $uploads['documento_bi']['tipo'];
    $comprovativo_type = $uploads['comprovativo']['tipo'];

    $stmt->bind_param(
        "ssssssibsbsbsss", // Corrected to 15 characters
        $nome_completo,
        $email,
        $telefone,
        $bi_numero,
    ...
    // Enviar dados BLOB usando send_long_data
    $stmt->send_long_data(7, $uploads['foto_passe']['conteudo']);
    $stmt->send_long_data(9, $uploads['documento_bi']['conteudo']);
    $stmt->send_long_data(11, $uploads['comprovativo']['conteudo']);
...
function processarUpload($field_name, $allowed_types) {
    if (!isset($_FILES[$field_name]) || $_FILES[$field_name]['error'] != UPLOAD_ERR_OK) {
        error_log("Erro no upload do arquivo $field_name: " . ($_FILES[$field_name]['error'] ?? 'Arquivo não enviado'));
        return false;
    }

    // Verificar tamanho do arquivo (limite de 2MB)
    if ($_FILES[$field_name]['size'] > 2097152) {
        error_log("Arquivo $field_name excede o tamanho máximo de 2MB");
        return false;
    }
    ...
    if (!in_array($file_type, $allowed_types)) {
        error_log("Tipo de arquivo inválido para $field_name: $file_type");
        return false;
    }
    ...
    if (!in_array($mime_type, $allowed_types)) {
        error_log("MIME type inválido para $field_name: $mime_type");
        return false;
    }
    ...
    if ($file_content === false) {
        error_log("Falha ao ler o conteúdo do arquivo $field_name");
        return false;
    }
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" id="rup" name="rup" readonly required>
                            <div class="error-text" id="rup_error"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-12">
                        <div class="copyright-content">
                            <p>© <?php echo date('Y'); ?> Instituto Politécnico 30 de Setembro. Todos os direitos reservados.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
     ...
     document.getElementById('telefone').addEventListener('input', function() {
        ...
        function gerarRUP() {
            let rupNumeros = '';
            for (let i = 0; i < 13; i++) {
                rupNumeros += Math.floor(Math.random() * 10);
            }
            ...
            if (inputRup) {
                inputRup.value = "RUP:" + rupFormatado;
            } else {
        ...
        // Chamar gerarRUP ao carregar a página
